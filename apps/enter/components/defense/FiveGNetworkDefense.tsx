...
interface FiveGNetworkDefenseProps {
...
export default function FiveGNetworkDefense({
  defenseData,
  isLoading,
  onBack,
  onStartDefense
}: FiveGNetworkDefenseProps) {
  // 状态管理
  ...
  useEffect(() => {
    ...
    const processFrame = () => {
      ...
      // 处理日志
      const logMap = {
        traffic_sample: '📊 5G流量采样更新',
        ddos_check: `5G DDoS 检测结果：${frame.is_ddos ? "⚠️ 有攻击" : "✅ 无异常"}`,
        ddos_source: `🚫 封锁恶意 IP：${frame.ip?.join(', ') || ''}，防御状态：${frame.success ? "成功" : "失败"}`,
        detection_model: `🧠 5G检测模型选择动作：${frame.message}`,
        defense_model: `🛡️ 5G防御模型选择动作：${frame.message}`,
        monitor_start: `📡 5G监控启动：${frame.description}`
      };
  ...
  return (
    <div className="p-6 h-full overflow-auto bg-gray-50">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">5G网络DDoS防御</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <Card title="5G网络流量收发情况" className="h-80">
        ...
        <Card title="5G网络检测过程日志">
          ...
          >
            ...
            {logs.length === 0 && (
              <div className="text-gray-500 text-center">等待开始5G网络检测...</div>
            )}
          </div>
        </Card>
      </div>
